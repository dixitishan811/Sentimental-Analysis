...
class Data:
    ...
    def get_tweets(self):
        raw_train = pd.read_csv('training.1600000.processed.noemoticon.csv',
                                header=None,
...
dt = Data()
pp = PreProcessing(dt, processing_size=1000)
we = WordEmbedding(pp, method='w2v_tfidf')
tr = Training(dt, pp, we, train_size=800, method='dt')
te = Testing(dt, pp, we, tr, test_size=200)
